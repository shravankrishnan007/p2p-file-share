...
export const UI = {
    ...
    // Helper to inject row into specific container (Room View)
    addFileEntryToContainer(container, id, name, size, isSender, handlers = {}) {
        ...
        // 2. Row HTML Structure (Matches CSS Grid)
        row.innerHTML = `
    ...
    // Updates UI when finished WITHOUT deleting the row content
    markFileComplete(id, isSender) {
        ...
        const progBar = row.querySelector(`#prog-${id}`);
        const speedText = row.querySelector(`#speed-${id}`);
        const actions = row.querySelector(`#actions-${id}`); // The buttons container
        ...
        // Remove buttons (Pause/Cancel) so user can't click them anymore
